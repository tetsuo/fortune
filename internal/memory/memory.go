...
func getCgroupStats() (map[string]uint64, error) {
	...
	readUintFile := func(filename string) (uint64, error) {
		data, err := os.ReadFile(filepath.Join(cgroupDir, filename))
		if err != nil {
			return 0, err
		}
	...
	m := map[string]uint64{}
	var err error
	m["high"], err = readUintFile("memory.high")
	if err != nil {
		return nil, err
	}
	m["max"], err = readUintFile("memory.max")
	if err != nil {
		return nil, err
	}
	m["current"], err = readUintFile("memory.current")
	if err != nil {
		return nil, err
	}
